...
async function triggerAchievementRecalculation(achievementId, database, productsService = null) {
  ...
  if (isRankingBased) {
    ...
    const usersWithActivity = await primaryDb
    ...
    const totalUsersResult = await primaryDb.select({ id: users.id }).from(users);
    console.log(`✅ Optimized: Checking ${allUsers.length} users with activity (saved checking ${totalUsersResult.length - allUsers.length} users with 0 activity)`);
...
router.get('/users', requireEmployeeAuth, async (req, res) => {
  ...
  try {
    ...
    console.log('👥 Fetching users with params:', { limit, offset, search });
    const result = await communityService.getAllUsersForAdmin({ limit, offset, search });
    console.log('👥 Returning', result.users.length, 'users');
...
// NOTE: Recalculate endpoint moved to server/routes/admin/recalculate.js
// to avoid route collision and handle engagement achievements properly
