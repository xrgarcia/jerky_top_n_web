...
- **User Profile**: 
  - **Private Profile Page** (`/profile`): Personal dashboard displaying "FirstName L." format regardless of privacy settings. Includes profile photo upload, handle management, and privacy controls. Privacy checkbox currently hidden for future rollout but infrastructure remains functional. Features "Edit Jerky.com Profile" button and disclaimers explaining that user's name comes from their jerky.com profile with links to edit at jerky.com/account. Dashboard sections include: stat cards (rankings, achievements, streak, points), "Your Jerky Journey" with progress tracking (achievements unlocked, current streak vs. best, unique flavors/animals ranked), and "Quick Actions" navigation shortcuts to Rank, Flavors, Coin Book, and Community with animated hover effects and earth-tone gradient styling.
  - **Profile Dashboard API** (`/api/profile/dashboard`): Comprehensive backend service orchestrating user insights via `ProfileDashboardService`. Combines personalized guidance (from `PersonalizedGuidanceService`), purchase analytics (unique products purchased, favorite product with image/metadata from `PurchaseHistoryRepository`), and gamification progress. Uses efficient Postgres aggregations (COUNT DISTINCT, SUM with MAX tie-break) for purchase statistics. Returns structured payload with null-safe defaults and graceful degradation when data unavailable. Frontend integration via `useProfileDashboard` React Query hook with proper caching strategy.
