...
- **Rank Page Products**: Server-side filtering via `/api/products/rankable` endpoint using `ProductRankingRepository` and `ProductsService.getRankableProductsForUser()` to exclude already-ranked products BEFORE pagination. Ensures users with extensive ranking history (70+ products) always see unranked products on initial load without manual pagination. **Critical**: All ranking operations use `rankingListId='default'` consistently across save/load/filter operations to prevent data mismatch. **Optimistic UI Pattern**: Products disappear instantly when ranked (before save completes) for zero-delay UX. State tracked in `optimisticallyRemovedProducts` array with automatic restoration on save failure. Both auto-save (800ms delay) and manual save clear tracking on success, restore products on failure with clear error notifications. **Server-side Search**: Type-ahead search (3+ characters) triggers server-side filtering that NEVER returns already-ranked products, ensuring search results always show only unranked products.
