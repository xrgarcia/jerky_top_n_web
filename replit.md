...
- **Database Connection Strategy**: Dual-connection architecture using Neon PostgreSQL. Most queries use the pooled connection (`server/db.js`) which routes through Neon's `-pooler` endpoint for load balancing and read replicas. **Critical-path queries** (achievement calculations in `CollectionManager`) use a dedicated primary-only connection (`server/db-primary.js`, max 5 connections) that bypasses the pooler to prevent read replica lag. This ensures achievement tier calculations always see the latest data immediately after users rank products, eliminating downgrade bugs caused by stale replica data. When instantiating `CollectionManager`, always pass `primaryDb` from `server/db-primary.js` instead of the pooled `db`.
