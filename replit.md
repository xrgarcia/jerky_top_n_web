...
- **Most Debated Products**: Identifies products with highest ranking variance (disagreement) using PostgreSQL STDDEV. Requires minimum 1 ranking per product with COALESCE handling NULL variance for single rankings (displays as 0.0 variance). Products sorted by variance descending to highlight community disagreement.
