...
- **Collection Progress Commentary**: Simple percentage-based encouragement messages in 10% tiers (0%, 1-10%, 11-20%, 21-30%, etc.) calculated from actual overall progress (rankedCount / totalProducts). Uses glossary terminology: "Flavor Coins" for individual products, "Coin Book" for the collection, and ownership language ("stash", "purchases") to emphasize products are already owned and waiting to be ranked. All tiers from 0% to 100% consistently reinforce the "already yours, just need ranking" framing aligned with the physical collection metaphor.
