...
- **Rank Page Products**: Server-side filtering via `/api/products/rankable` endpoint using `ProductRankingRepository` and `ProductsService.getRankableProductsForUser()` to exclude already-ranked products BEFORE pagination. Ensures users with extensive ranking history (70+ products) always see unranked products on initial load without manual pagination. **Optimistic UI Pattern**: Products disappear instantly when ranked (before save completes) for zero-delay UX. State tracked in `optimisticallyRemovedProducts` array with automatic restoration on save failure. Both auto-save (800ms delay) and manual save clear tracking on success, restore products on failure with clear error notifications. Provides instant visual feedback while maintaining complete error recovery.
