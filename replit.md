...
- **Icon Rendering**: Unified icon utility (`src/utils/iconUtils.jsx`) handles all achievement icon types (emoji, URL, base64) consistently across all components. The `getAssetUrl()` helper normalizes relative paths by prepending the API origin in development (via `VITE_API_ORIGIN` env var) and falling back to same-origin in production.
