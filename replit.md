...
## Recent Changes
- 2025-10-11: **Fixed flavor vs meat categorization** - Resolved issue where "Buffalo Wing" flavor was incorrectly categorized as Buffalo meat instead of Chicken; updated animal extraction logic with 3-tier priority system (primary meat types → multi-word animals → other animals); ensures product meat type is identified before flavor names; Chicken category now correctly shows 4 products (including Buffalo Wing flavor), Buffalo category reduced to 3 products (actual buffalo meat only)
- 2025-10-11: **Unified ProductsService architecture** - Created centralized ProductsService class that combines Shopify products, metadata, and ranking stats into single source of truth; refactored /api/products/all endpoint from ~150 lines to ~50 lines by eliminating manual data merging; fixed sorting bug where pork filter showed only 2 products instead of 8 - frontend now uses metadata-first filtering with fallback to title/tags search in both sort field and sort order handlers; dependency injection pattern with shared cache instances ensures data consistency across all operations; all 8 pork products (2 pork jerky + 6 bacon jerky) now display correctly when filtering and sorting
